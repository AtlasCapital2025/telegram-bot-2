...
async def check_subscription(target, context, user_id):
    ...
    try:
        ...
        print(f"Проверка подписки: статус {member.status}")

        if member.status in ["member", "administrator", "creator"]:
            print("✅ Пользователь подписан, отправляю PDF...")

            if not os.path.isfile(GUIDE_FILE_PATH):
                await target.reply_text("Файл не найден. Убедитесь, что он загружен в проект.")
                return
...
def main():
    ...
    application.add_handler(
        MessageHandler(filters.TEXT & filters.ChatType.PRIVATE & ~filters.Regex("^Старт$"), start)
    )
